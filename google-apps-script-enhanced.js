...
function importProducts(data) {
    try {
        ...
        for (let i = 1; i < productData.length; i++) {
            if (productData[i][1] === productCode) { // Cột B - Mã Sản Phẩm
        ...
        if (!tenSP) {
            return createResponse({ error: "Không tìm thấy sản phẩm với mã: " + productCode }, 400);
        }
        ...
        importSheet.appendRow([currentTime, tenSP, quantity, "Tự động tạo mã vạch"]);
        ...
        barcodes.forEach((barcode) => {
            barcodeSheet.appendRow([barcode, productCode, tenSP, currentTime, "Trong Kho"]);
        });
